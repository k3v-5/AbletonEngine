...
# 1. Spectral Freeze Drone
def mutate_01_spectral_freeze(audio: np.ndarray, sr: int) -> np.ndarray:
...
# 2. Tuned Comb Karplus-Strong Chime
def mutate_02_tuned_comb_chime(audio: np.ndarray, sr: int) -> np.ndarray:
    # Comb filter delays tuned to Fm chord: F3 (174.6 Hz), Ab3 (207.65 Hz), C4 (261.63 Hz)
    freqs = [174.61, 207.65, 261.63]
    out = np.zeros_like(audio)
    feedback = 0.88
...
# 3. Vocal Formant Resonance
def mutate_03_vocal_formant_resonance(audio: np.ndarray, sr: int) -> np.ndarray:
...
# 4. Industrial Multi-Stage Wavefolder
def mutate_04_industrial_crunch_mutation(audio: np.ndarray, sr: int) -> np.ndarray:
...
# 5. Sub-Safe Low-End Saturated Growl
def mutate_05_sub_safe_low_growl(audio: np.ndarray, sr: int) -> np.ndarray:
    ...
    # Generate sub-octave fundamental (-12st) via half-frequency phase follower
    t = np.linspace(0, len(audio) / sr, len(audio), endpoint=False)[:, np.newaxis]
    sub_octave = np.sin(2.0 * np.pi * 43.65 * t) * np.abs(mono_sub) * 1.4  # F1 = 43.65 Hz
...
# 6. Pitch-Shifted Shimmer Diffusion
def mutate_06_pitch_shimmer_diffusion(audio: np.ndarray, sr: int) -> np.ndarray:
...
# 7. Dark Reese Double-Octave Dive
def mutate_07_dark_reese_octave_dive(audio: np.ndarray, sr: int) -> np.ndarray:
...
# 8. Granular Micro-Particle Cloud
def mutate_08_granular_micro_cloud(audio: np.ndarray, sr: int) -> np.ndarray:
...
# 9. Vintage Cassette Wow & Flutter
def mutate_09_vintage_tape_wow_warp(audio: np.ndarray, sr: int) -> np.ndarray:
...
# 10. Inharmonic Frequency-Shifted Bell Ring
def mutate_09_inharmonic_metallic_ring(audio: np.ndarray, sr: int) -> np.ndarray:
    ...
    # Metallic ring modulation with 55Hz sub-carrier
    ring = out * np.sin(2.0 * np.pi * 55.0 * t) * 0.5
    out = out * 0.6 + ring * 0.4
    return normalize_audio(out, -1.0)


# 11. Reverse Swell Exponential Bloom
def mutate_11_reverse_swell_bloom(audio: np.ndarray, sr: int) -> np.ndarray:
    # 1. Reverse audio
    rev = audio[::-1, :]
...
# 12. 10-Bit Downsampled Digital Dirt
def mutate_12_lofi_bit_crusher_dirt(audio: np.ndarray, sr: int) -> np.ndarray:
...
# 13. Haas 3D Psychoacoustic Decoupler
def mutate_13_haas_3d_spatial_decouple(audio: np.ndarray, sr: int) -> np.ndarray:
    # Mid / Side encoding
...
# 14. Syncopated Rhythmic Stutter Slicer
def mutate_14_rhythmic_stutter_chop(audio: np.ndarray, sr: int) -> np.ndarray:
    # 16th-note slicing (125ms per 16th at 120 BPM)
    sixteenth = int(sr * 0.125)
    out = np.zeros_like(audio)
...
# 15. Full-Wave Octave Fuzz Multiplier
def mutate_15_octave_fuzz_multiplier(audio: np.ndarray, sr: int) -> np.ndarray:
...
# 16. Chopped Polyrhythmic Trance Pulse
def mutate_16_chopped_trance_pulse(audio: np.ndarray, sr: int) -> np.ndarray:
    t = np.linspace(0, len(audio) / sr, len(audio), endpoint=False)[:, np.newaxis]
    # 16th-note square pulse gate with soft trapezoidal edges
    pulse_freq = 8.0  # 8 pulses per second = 16th notes at 120 BPM
    gate = 0.5 * (1.0 + np.sin(2.0 * np.pi * pulse_freq * t))
    gate = np.clip(gate * 3.0, 0.0, 1.0)
    
    gated = audio * gate
    
    # Stereo ping-pong bounce
    delay_l = int(sr * 0.1875)  # dotted 16th
    delay_r = int(sr * 0.250)   # 8th
...
# 17. Spectral Gaussian Blur Infinite
def mutate_17_spectral_blur_infinite(audio: np.ndarray, sr: int) -> np.ndarray:
    n_fft = 4096
    hop = 512
    out = np.zeros_like(audio)
...
# 18. Analog Tape Warmth & Opto Glue
def mutate_18_analog_tape_warmth_glue(audio: np.ndarray, sr: int) -> np.ndarray:
...
# 19. Neoperreo Resonant Metallic Comb
def mutate_19_neoperreo_metallic_comb(audio: np.ndarray, sr: int) -> np.ndarray:
...
# 20. Exponential Pitch Dive & HPF Sweep
def mutate_20_exponential_pitch_dive(audio: np.ndarray, sr: int) -> np.ndarray:
...
MUTATION_REGISTRY = [
    ...
    ("INHARMONIC_METALLIC_RING", mutate_09_inharmonic_metallic_ring),
...
def render_all_20_mutations(source_wav_path: str, output_dir: str) -> list:
    ...
    print(f"Sample Rate: {sr} Hz, Duration: {len(audio)/sr:.1f}s ({len(audio)} samples)\n")
